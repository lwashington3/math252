...
1st Order D.E. in differential form \[ M(x,y)dx+N(x,y)dy=0 \] Given a function \[ z=f(x,y) \], the total differential, $dz$, is defined as \[ dx = \frac{\partial f}{\partial x}dx + \frac{\partial f}{\partial y}dy \]

\subsection{Method}\label{subsec:exact-equations-method}
See if whe can find a function $f(x,y)$ such that \[ \frac{\partial f}{\partial x}=M, \frac{\partial f}{\partial y}=N \]
...
\begin{aligned}
	...
	\end{array} \right\} \mbox{ Theorem tells use these are equal}
\end{aligned}
\end{equation*}
...
\end{aligned}
\end{equation*} \textbf{Note: the $f_{M}$ format is just there to show which partial equation was integrated. It was made by me and, as far as I know, not standardly known.}
