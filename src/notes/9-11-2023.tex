...
\noindent\definition{Linear combination\label{dfn:linear-combination}}{a collection of solutions $y_{1}, y_{2}, \dots, y_{m}$ is linearly independent is if the only way $c_{1}y_{1} + c_{2}y_{2} + \dots + c_{m}y_{m}=0$ is iff (if and only if) all of the constants $c_{1}, c_{2}, \dots, c_{m}=0$. Otherwise we say $y_{1}, y_{2}, \dots, y_{m}$ are linearly dependent.}\\\\
