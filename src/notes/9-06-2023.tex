...
\theorem{Given a D.E. \[ M(x,y)dx + N(x,y)dy = 0 \] A function $f(x,y)$ is said to be homogenous of order $\alpha$ if $f(tx, ty)=t^{\alpha}f(x,y)$}.
...
\begin{equation*}
\begin{aligned}
	M(x,y) = x^{2} + y^{2} \mbox{   } & \mbox{   } N = x^{2}-xy \\
	M_{y} = 2y \mbox{   } & \mbox{   } N_{x} = 2x-y \\
	M_{y} &\neq N_{x}\\
	M(tx,ty) &= (tx)^{2} + (ty)^{2}\\
			 &= t^{2}x^{2} + t^{2}y^{2}\\
			 &= t^{2}(x^{2} + y^{2})\\
...
\section{Bernoulli Equation}\label{sec:bernoulli-equation}
